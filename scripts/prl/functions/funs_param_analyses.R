...
add_subj_code_to_hddm_params_and_save <- function() {
  ...
  lookup_tbl <- rio::import(
    here::here("data", "processed", "_prl", "input_for_hddmrl", "hddm_look_up_table_20230105.csv")
  ) %>% 
  ...
  params_prl_ddm <- rio::import(
    here::here("data", "processed", "_prl", "output_hddm", "output_2023_03_09.txt")
  )
  ...
  params_prl_ddm$is_hoarding <- stringr::str_detect(params_prl_ddm$ii, "hoarding")
  ...
  params_prl_ddm <- params_prl_ddm %>% 
    mutate(stim = case_when(
      is_hoarding == TRUE  & is_neutral == FALSE ~ "hoarding",      
      is_hoarding == FALSE & is_neutral == TRUE  ~ "neutral",  
      TRUE                                  ~ "neither")) 
  ...
  alpha_neg_df <- wrangle_params(params_prl_ddm_clean, lookup_tbl, 
                         "alpha_neg_neutral", "alpha_neg_hoarding")
  ...
  alpha_pos_df <- wrangle_params(params_prl_ddm_clean, lookup_tbl, 
                                 "alpha_pos_neutral", "alpha_pos_hoarding")
...
wrangle_params <- function(params_prl_ddm_clean, lookup_tbl,
  ...
  hoarding_df <- params_prl_ddm_clean %>%
    dplyr::filter(pp == PARAM_HOARDING)
  hoarding_df$stim <- "hoarding"
  hoarding_df <- hoarding_df %>%
    dplyr::select(subj_idx, stim, value)

  df <- bind_rows(neutral_df, hoarding_df)
