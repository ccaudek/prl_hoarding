# Script name: funs_param_analyses_vdp.R
...
#' @save
#' RDS file.
add_subj_code_to_hddm_params_and_save_vdp <- function() {
  ...
  lookup_tbl <- rio::import(
    here::here("data", "raw", "villadeipini", "prl", "hddm_look_up_table_20220920.csv")
  ) %>% 
  ...
  params_prl_ddm <- rio::import(
    here::here("data", "processed", "villadeipini", "prl", 
               "villadeipini_params_hddmrl.txt")
  )
  ...
  params_prl_ddm_clean <- params_prl_ddm %>% 
    dplyr::select(
      subj_idx, params, mean
    ) %>% 
    dplyr::rename(
      value = mean
    )
  
  params_prl_ddm_clean$pp <- paste(params_prl_ddm_clean$params, sep="_")
  ...
  t_df <- wrangle_params(params_prl_ddm_clean, lookup_tbl, "t")
  t_df <- t_df %>% 
    dplyr::rename(
      t = value
    )
  
  a_df <- wrangle_params(params_prl_ddm_clean, lookup_tbl, "a")
  a_df <- a_df %>% 
    dplyr::rename(
      a = value
    )
  
  v_df <- wrangle_params(params_prl_ddm_clean, lookup_tbl, "v")
  v_df <- v_df %>% 
    dplyr::rename(
      v = value
    )
  
  alpha_neg_df <- wrangle_params(params_prl_ddm_clean, lookup_tbl, "alpha_neg")
  alpha_neg_df <- alpha_neg_df %>% 
    dplyr::rename(
      alpha_neg = value
    )
  
  alpha_pos_df <- wrangle_params(params_prl_ddm_clean, lookup_tbl, "alpha_pos")
  ...
  temp1 <- inner_join(t_df, a_df, by = c("subj_code", "subj_idx"))
  temp2 <- inner_join(temp1, v_df, by = c("subj_code", "subj_idx"))
  temp3 <- inner_join(temp2, alpha_neg_df, by = c("subj_code", "subj_idx"))
  param_final <- inner_join(temp3, alpha_pos_df, by = c("subj_code", "subj_idx"))

  saveRDS(
    param_final,
    here::here("data", "processed", "villadeipini", "prl", 
               "prl_vdp_params_and_subj_code.rds")
  )
...
#' @return 
#' Dataframe.
wrangle_params <- function(params_prl_ddm_clean, lookup_tbl, PARAM) {

  neutral_df <- params_prl_ddm_clean %>%
    dplyr::filter(pp == PARAM)
  
  df <- neutral_df %>%
    dplyr::select(subj_idx, value)
